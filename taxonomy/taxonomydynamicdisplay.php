...
include_once($SERVER_ROOT.'/content/lang/taxa/taxonomy/taxonomydynamicdisplay.'.$LANG_TAG.'.php');
...
<html>
<head>
	<title><?php echo $DEFAULT_TITLE." ".$LANG['TAXONOMY_EXPLORER']." : ".$taxonDisplayObj->getTargetStr(); ?></title>
...
if(isset($taxa_admin_taxonomydisplayCrumbs)){
	...
	echo " <b>".$LANG['TAXONOMY_EXPLORER']."</b>";
	echo "</div>";
}
if(isset($taxa_admin_taxonomydisplayCrumbs)){
	...
	<div class="navpath">
		...
		<a href="taxonomydynamicdisplay.php"><b><?php echo $LANG['TAXONOMY_EXPLORER']; ?></b></a>
	</div>
	<?php
}
?>
	...
	<div id="innertext">
		...
		if($editable){
			?>
			<div style="float:right;" title="<?php echo $LANG['ADD_A_NEW_TAXON']; ?>">
		...
		<div>
			<form id="tdform" name="tdform" action="taxonomydynamicdisplay.php" method='POST'>
				<fieldset style="padding:10px;width:500px;">
					<legend><b><?php echo $LANG['ENTER_TAX']; ?></b></legend>
                    <div>
					...
					<div style="float:right;margin:15px 80px 15px 15px;">
						<input type="hidden" name="tdsubmit" value="Display Taxon Tree" />
						<input type="submit" value="<?php echo $LANG['DISPLAY_TAXON_TREE']; ?>"/>
						<input name="taxauthid" type="hidden" value="<?php echo $taxAuthId; ?>" />
					</div>
					<div style="margin:15px 15px 0px 60px;">
						<input name="displayauthor" type="checkbox" value="1" <?php echo ($displayAuthor?'checked':''); ?> /> <?php echo $LANG['AUTHOR_VIEW']; ?>
					</div>
				</fieldset>
			</form>
		</div>
